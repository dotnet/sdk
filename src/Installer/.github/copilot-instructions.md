...
Error handling in dotnetup commands (telemetry contract):
- dotnetup commands MUST signal failure by throwing `DotnetInstallException` (with the most specific `DotnetInstallErrorCode` available). `ExecuteCore` returns `void` on success — there is no "return non-zero to signal failure" path. `CommandBase.Execute` is the single chokepoint that converts an exception into the process exit code AND calls `DotnetupTelemetry.RecordException`, which is what stamps `error.type` / `error.category` / `error.hresult` / `error.details` onto the command's telemetry row. Returning successfully without throwing produces a `(no error.type)` row in the dashboard with exit code 0.
- The narrow exception is `DotnetCommand` / `ElevatedSystemPathCommand`, which forward a child process's exit code via `SetExitCode(int)`. Do NOT use `SetExitCode` as a substitute for throwing on failures (that bypasses the telemetry pipeline).
- This applies to every non-success path, including pure validation failures (e.g. unsupported platform, not elevated, dotnet executable not found, UAC prompt cancelled, no matching uninstall target, unsupported shell). Pick the closest `DotnetInstallErrorCode` — `PlatformNotSupported`, `PermissionDenied`, `ContextResolutionFailed`, `UninstallTargetNotFound`, `VersionNotFound`, `DownloadFailed`, `Unknown`, etc. — so `ErrorCategoryClassifier` can route it to Product vs User correctly. If a new failure mode doesn't fit any existing code, ADD a new enum value rather than reusing `Unknown`.
- Do NOT add `catch (Exception) { ... return 1; }` inside a command — that re-implements `CommandBase`'s catch but skips telemetry. The only legitimate reason to catch in a command is to do side-effects (e.g. write to a shared IPC log file in `ElevatedSystemPathCommand`) and then **rethrow** with `throw;`.
- Concurrency / batch caveat: workflows that operate on multiple independent items (the `sdk update` / `runtime update` flows installing across multiple roots, specs, or components) MUST keep going after one item fails so the remaining items still get installed/updated. The pattern is:
  1. Loop over items inside a `try { ... } catch (DotnetInstallException ex) { failureCount++; firstFailure ??= ExceptionDispatchInfo.Capture(ex); }`.
  2. After the loop, if `firstFailure is not null`, optionally print a summary like `"{N} update(s) failed; reporting the first failure."` and call `firstFailure.Throw()` so `CommandBase` records it.
  Reference implementations: `UpdateWorkflow.Execute` / `UpdateWorkflow.UpdateRoot` and `RuntimeUpdateCommand.ExecuteCore`. When editing those files (or adding a similar batch flow), preserve this best-effort-then-rethrow shape — do NOT short-circuit on the first failure, and do NOT swallow subsequent failures by returning a non-zero exit code without rethrowing.
