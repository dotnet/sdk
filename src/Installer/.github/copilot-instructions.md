...
- dotnetup commands MUST signal failure by throwing `DotnetInstallException` (with the most specific `DotnetInstallErrorCode` available), NOT by `return 1` from `ExecuteCore` or any helper. `CommandBase.Execute` is the single chokepoint that converts an exception into the process exit code AND calls `DotnetupTelemetry.RecordException`, which is what stamps `error.type` / `error.category` / `error.hresult` / `error.details` onto the command's telemetry row. A bare `return 1` skips that pipeline and produces a `(no error.type)` row in the dashboard.
