...
- The narrow exception is `DotnetCommand` / `ElevatedAdminPathCommand`, which forward a child process's exit code via `SetExitCode(int)`. Do NOT use `SetExitCode` as a substitute for throwing on failures (that bypasses the telemetry pipeline).
...
- Do NOT add `catch (Exception) { ... return 1; }` inside a command — that re-implements `CommandBase`'s catch but skips telemetry. The only legitimate reason to catch in a command is to do side-effects (e.g. write to a shared IPC log file in `ElevatedAdminPathCommand`) and then **rethrow** with `throw;`.
