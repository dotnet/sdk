...
/// <summary>
/// Inspects the <em>persisted</em> environment and reports what dotnetup has actually wired as an
/// <see cref="ObservedEnvironmentState"/>. This is the single place that reads machine reality
/// for the env-axis model, so both <see cref="EnvSettingsApplier"/> (for reality-based removal) and
/// <see cref="Commands.Env.EnvShowCommand"/> (for drift reporting) make decisions from the same
/// observation instead of trusting the stored config. Mirrors the inspect-then-act split of
...
internal sealed class EnvironmentStateInspector : IEnvironmentStateInspector
{
    ...
    /// supplied (or can be detected); when it is <c>null</c>, <see cref="ObservedEnvironmentState.ProfileBlock"/>
    /// is left <see cref="ProfileBlockState.Unknown"/> to signal "unknown" rather than asserting absence.
    /// </summary>
    public ObservedEnvironmentState Inspect(IEnvShellProvider? shellProvider)
    {
        ...
        ProfileBlockState profileBlock = shellProvider is null
            ? ProfileBlockState.Unknown
            : ShellProfileManager.GetProfilePathsWithEntries(shellProvider).Count > 0
                ? ProfileBlockState.Present
                : ProfileBlockState.Absent;
