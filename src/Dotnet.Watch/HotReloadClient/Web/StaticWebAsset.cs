...
internal readonly struct StaticWebAsset(string filePath, string relativeUrl, string assemblyName, bool isApplicationProject)
{
    ...
    /// <summary>
    /// The browser tools activation initializer, the browser tools client and its generated
    /// configuration are build outputs that Microsoft.NET.Sdk.StaticWebAssets.DotNetWatch.targets
    /// writes to '$(IntermediateOutputPath)dotnet-watch'. They are stable for the lifetime of the
    /// watch invocation, so watching them would only add noise and cannot produce a useful update.
    /// </summary>
    public static bool IsBrowserToolsAssetFile(string filePath)
        => Path.GetFileName(filePath).IndexOf(".DotNetWatch.", StringComparison.Ordinal) >= 0 &&
           string.Equals(Path.GetFileName(Path.GetDirectoryName(filePath)), "dotnet-watch", OSSpecificPathComparison);

    /// <summary>
    /// The JS initializer manifest is regenerated by the build from the set of JS module assets.
    /// It is not editable, so watching it would only report the build's own output back to itself.
    /// </summary>
    public static bool IsGeneratedJSModuleManifestFile(string filePath)
    {
        var fileName = Path.GetFileName(filePath);

        return fileName.StartsWith("jsmodules.", StringComparison.OrdinalIgnoreCase) &&
