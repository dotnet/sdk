...
namespace Microsoft.DotNet.PackageInstall.Tests
{
    public class NuGetPackageInstallerTests : SdkTest
    {
        ...
        public async Task GivenNoPackageSourceMappingItShouldError()
        {
            ...
            NuGetPackageInstallerException ex = await ThrowsAsync(() => _toolInstaller.DownloadPackageAsync(
                TestPackageId,
                new NuGetVersion(TestPackageVersion),
                new PackageSourceLocation(sourceFeedOverrides: new[] { relativePath }),
                packageSourceMapping: mockPackageSourceMapping));
            ex.Message.Should().Contain(string.Format(Cli.NuGetPackageDownloader.LocalizableStrings.FailedToFindSourceUnderPackageSourceMapping, TestPackageId));
        }
        ...
        public async Task GivenPackageSourceMappingFeedNotFoundItShouldError()
        {
            ...
            NuGetPackageInstallerException ex = await ThrowsAsync<NugGetPackageInstallerException>(() => _toolInstaller.DownloadPackageAsync(
                TestPackageId,
                new NuGetVersion(TestPackageVersion),
                new PackageSourceLocation(sourceFeedOverrides: new[] { relativePath }),
                packageSourceMapping: mockPackageSourceMapping));
            ex.Message.Should().Contain(string.Format(Cli.NuGetPackageDownloader.LocalizableStrings.FailedToMapSourceUnderPackageSourceMapping, TestPackageId));
        }
