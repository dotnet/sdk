    ...
    public class GivenDotnetCleanCleansBuildArtifacts : SdkTest
    {
        ...
        public void ItCleansAProjectBuiltWithRuntimeIdentifier()
        {
            ...
            new DotnetBuildCommand(Log, testInstance.Path)
                .Execute("-r", $"{ToolsetInfo.LastWinRuntimeIdentifier}-x64")
                .Should().Pass();

            var configuration = Environment.GetEnvironmentVariable("CONFIGURATION") ?? "Debug";
            var outputFolder = new DirectoryInfo(Path.Combine(testInstance.Path, "bin", configuration, ToolsetInfo.CurrentTargetFramework, $"{ToolsetInfo.LastWinRuntimeIdentifier}-x64"));
