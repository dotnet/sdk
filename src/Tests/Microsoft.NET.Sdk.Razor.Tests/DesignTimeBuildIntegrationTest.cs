...
namespace Microsoft.AspNetCore.Razor.Design.IntegrationTests
{
    public class DesignTimeBuildIntegrationTest : SdkTest
    {
        ...
        public void RazorGenerateComponentDesignTime_ReturnsRazorComponentWithTargetPath()
        {
            ...
            var command = new MSBuildCommand(Log, "RazorGenerateDesignTime;_IntrospectRazorComponentWithTargetPath", projectDirectory.Path);
            var result = command.Execute();
            result.Should().Pass();
            ...
            {
                result.Should().HaveStdOutContaining(
                    $@"RazorComponentWithTargetPath: {filePath} {filePath} {Path.Combine("obj", "Debug", "net5.0", "Razor", filePath + ".g.cs")} {Path.Combine("obj", "Debug", "net5.0", "Razor", filePath + ".g.cs")}");
            }
        }
    }
}
