...
public class DockerDaemonTests
{
    ...
    public async Task Can_detect_when_no_daemon_is_running() {
        ...
        try {
            ...
            var available = await new LocalDocker(message => Console.WriteLine($"{message.level}: {String.Format(message.messageFormat, message.formatArgs)}")).IsAvailableAsync(default).ConfigureAwait(false);
