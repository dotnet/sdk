...
namespace Microsoft.DotNet.ApiSymbolExtensions
{
    ...
    public class AssemblySymbolLoader : IAssemblySymbolLoader
    {
        ...
        private MetadataReference CreateAndAddReferenceToCompilation(string assemblyName, string rootAssemblyDisplayString, Stream fileStream, ImmutableHashSet<string>? referenceAssemblyNamesToIgnore = null)
        {
        ...
        private void ResolveReferences(PEReader peReader, string assemblyName, string rootAssemblyDisplayString, ImmutableHashSet<string>? referenceAssemblyNamesToIgnore = null)
        {
            ...
            foreach (AssemblyReferenceHandle assemblyReferenceHandle in reader.AssemblyReferences)
            {
                ...
                if (_referencePathFiles.TryGetValue(assemblyReferenceName, out string? fullReferencePath))
                {
                    ...
                    CreateAndAddReferenceToCompilation(assemblyReferenceName, rootAssemblyDisplayString, resolvedStream, referenceAssemblyNamesToIgnore);
                }
                ...
                else
                {
                    ...
                    if (!found)
                    {
                        _log.LogWarning(AssemblyReferenceNotFoundErrorCode,
                            string.Format(Resources.CouldNotResolveReference, assemblyReferenceName, assemblyName, rootAssemblyDisplayString));
                    }
                }
            }
        }
