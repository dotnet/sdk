...
<Compile Include="Program.cs" /> <!-- project-local AOT entry point -->
...
The two entry points are deliberately **not** shared. Each project owns its own
`Program.cs`:

- **`src/Cli/dotnet-aot/Program.cs`** — the NativeAOT `Program.Main`: a simple
  delegate to the shared `Parser`. The per-invocation telemetry/signal/first-run
  setup is driven from `NativeEntryPoint` instead of a static constructor.
- **`src/Cli/dotnet/Program.cs`** — the managed CLI entry point with telemetry,
  signal handlers, and workload checks. It no longer carries any `#if CLI_AOT`
  branches and is not linked into the AOT build.
