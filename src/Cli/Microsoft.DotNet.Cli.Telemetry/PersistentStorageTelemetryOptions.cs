...
public sealed class PersistentStorageTelemetryOptions
{
    ...
    /// <summary>
    /// Whether the exporter starts its own in-process background worker when telemetry is first
    /// persisted. The worker remains available for the exporter lifetime and drains again whenever
    /// later telemetry is persisted.
