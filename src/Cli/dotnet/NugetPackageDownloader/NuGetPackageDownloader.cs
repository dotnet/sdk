...
internal class NuGetPackageDownloader : INuGetPackageDownloader
{
        ...
        PackageSourceMapping packageSourceMapping = null)
    {
        ...
        if (resource == null)
        {
            throw new NuGetPackageNotFoundException(
                string.Format(CommonLocalizableStrings.IsNotFoundInNuGetFeeds, packageId, source.Source));
        }
    ...
    private async Task VerifySigning(string nupkgPath, SourceRepository repository)
    {
        if (!_verifySignatures && !_validationMessagesDisplayed)
        {
            ...
            {
                _reporter.WriteLine(CommonLocalizableStrings.NuGetPackageSignatureVerificationSkipped);
            }
            ...
            resource.AllRepositorySigned)
        {
            ...
            {
                throw new NuGetPackageInstallerException(string.Format(CommonLocalizableStrings.FailedToValidatePackageSigning, commandOutput));
            }

            if (DiagnosticVerbosity())
            {
                _reporter.WriteLine(CommonLocalizableStrings.VerifyingNuGetPackageSignature, Path.GetFileNameWithoutExtension(nupkgPath));
            }
        }
        else if (DiagnosticVerbosity())
        {
            _reporter.WriteLine(CommonLocalizableStrings.NuGetPackageShouldNotBeSigned, Path.GetFileNameWithoutExtension(nupkgPath));
        }
    }
        ...
        foreach (string source in packageSourceLocation?.SourceFeedOverrides)
        {
            ...
            if (packageSource.TrySourceAsUri == null)
            {
                _verboseLogger.LogWarning(string.Format(
                    CommonLocalizableStrings.FailedToLoadNuGetSource,
                    source));
                continue;
            }
    ...
    private List<PackageSource> LoadDefaultSources(PackageId packageId, PackageSourceLocation packageSourceLocation = null, PackageSourceMapping packageSourceMapping = null)
    {
        ...
        if (_shouldUsePackageSourceMapping && packageSourceMapping?.IsEnabled == true)
        {
            ...
            if (sources.Count == 0)
            {
                throw new NuGetPackageInstallerException(string.Format(CommonLocalizableStrings.FailedToFindSourceUnderPackageSourceMapping, packageId));
            }
            defaultSources = [.. defaultSources.Where(source => sources.Contains(source.Name))];
            if (defaultSources.Count == 0)
            {
                throw new NuGetPackageInstallerException(string.Format(CommonLocalizableStrings.FailedToMapSourceUnderPackageSourceMapping, packageId));
            }
        }
        ...
        CancellationToken cancellationToken)
    {
        ...
        else
        {
            throw new NuGetPackageNotFoundException(
                string.Format(
                    CommonLocalizableStrings.IsNotFoundInNuGetFeeds,
    ...
    private static string GenerateVersionRangeErrorDescription(string packageIdentifier, VersionRange versionRange)
    {
        ...
        {
            return string.Format(CommonLocalizableStrings.PackageVersionDescriptionForExactVersionMatch,
                versionRange.MinVersion, packageIdentifier);
        }
        else if (versionRange.HasLowerAndUpperBounds)
        {
            return string.Format(CommonLocalizableStrings.PackageVersionDescriptionForVersionWithLowerAndUpperBounds,
                versionRange.MinVersion, versionRange.MaxVersion, packageIdentifier);
        }
        else if (versionRange.HasLowerBound)
        {
            return string.Format(CommonLocalizableStrings.PackageVersionDescriptionForVersionWithLowerBound,
                versionRange.MinVersion, packageIdentifier);
        }
        else if (versionRange.HasUpperBound)
        {
            return string.Format(CommonLocalizableStrings.PackageVersionDescriptionForVersionWithUpperBound,
                versionRange.MaxVersion, packageIdentifier);
        }

        // Default message if the format doesn't match any of the expected cases
        return string.Format(CommonLocalizableStrings.PackageVersionDescriptionDefault, versionRange, packageIdentifier);
    }
        ...
        string packageIdentifier, IEnumerable<PackageSource> packageSources, bool includePreview, CancellationToken cancellationToken, int numberOfResults)
    {
        ...
        {
            throw new NuGetPackageNotFoundException(
                string.Format(CommonLocalizableStrings.IsNotFoundInNuGetFeeds, packageIdentifier, packageSources.Select(s => s.Source)));
        }
        ...
        NuGetVersion packageVersion, IEnumerable<PackageSource> sources, CancellationToken cancellationToken, bool includeUnlisted = false)
    {
        ...
        {
            throw new NuGetPackageInstallerException(string.Format(CommonLocalizableStrings.FailedToLoadNuGetSource,
                string.Join(";", sources.Select(s => s.Source))));
        }

        throw new NuGetPackageNotFoundException(string.Format(CommonLocalizableStrings.IsNotFoundInNuGetFeeds,
