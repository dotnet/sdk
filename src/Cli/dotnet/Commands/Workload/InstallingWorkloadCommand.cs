...
internal abstract class InstallingWorkloadCommand : WorkloadCommandBase
{
    ...
    /// referenced in the workload set exist on disk. This should be called early in command
    /// execution to repair corrupt state before attempting to use the workload resolver.
    /// </summary>
    /// <returns>The corrupt WorkloadSet if any manifests are missing; null otherwise.</returns>
    protected WorkloadSet GetCorruptWorkloadSetIfAny()
    {
        var installState = GetInstallState();

        // Only check for corruption if using workload sets mode
        if (!installState.ShouldUseWorkloadSets())
        {
            return null;
        }

        // Must have a workload set version specified to check for corruption
        ...
        try
        {
            ...
            return HasMissingManifests(workloadSet) ? workloadSet : null;
        }
        catch
        {
            // If we can't read the workload set, consider it not corrupt
            // (it might not be installed at all - maybe we should consider removing it)
            return null;
        }
    }
    ...
    /// </summary>
    protected bool HasMissingManifests(WorkloadSet workloadSet)
    {
    ...
    /// </summary>
    protected void RepairCorruptWorkloadSet(ITransactionContext context, WorkloadSet workloadSet)
    {
        var installState = GetInstallState();
    ...
    /// <summary>
    /// Reads the workload install state from disk for the current SDK feature band.
    /// </summary>
    protected InstallStateContents GetInstallState()
    {
    ...
    /// </summary>
    protected IEnumerable<ManifestVersionUpdate> CreateManifestUpdatesFromWorkloadSet(WorkloadSet workloadSet)
    {
