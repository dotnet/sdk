...
internal sealed class ProjectConvertCommand : CommandBase<ProjectConvertCommandDefinition>
{
    ...
    public override int Execute()
    {
        ...
        var builder = new VirtualProjectBuilder(BuildService.Instance, LanguageService.Instance, file, VirtualProjectBuildingCommand.TargetFramework);
        ...
        ConvertedFile ConvertFile(string sourceFile, string outputDirectory, bool isEntryPointFile)
        {
            ...
            {
                fileBuilder = new VirtualProjectBuilder(BuildService.Instance, LanguageService.Instance, sourceFile, VirtualProjectBuildingCommand.TargetFramework);
        ...
        void ValidateRefTargetDirectories(ImmutableArray<CSharpDirective> directives, string sourceDirectory, HashSet<string> visited, HashSet<string> usedFolderNames)
        {
            foreach (var directive in directives)
            {
                ...
                var refBuilder = new VirtualProjectBuilder(BuildService.Instance, LanguageService.Instance, refPath, VirtualProjectBuildingCommand.TargetFramework);
