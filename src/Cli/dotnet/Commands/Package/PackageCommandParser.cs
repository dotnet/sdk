...
{
    private static readonly Command Command = SetActionAndCompletions(new PackageCommandDefinition());
    ...
    private static Command SetActionAndCompletions(PackageCommandDefinition def)
    {
        ...
        def.AddCommand.PackageIdArgument.CompletionSources.Add(context =>
        {
            // we should take --prerelease flags into account for version completion
            var allowPrerelease = context.ParseResult.GetValue(def.AddCommand.PrereleaseOption);
        ...
        def.AddCommand.VersionOption.CompletionSources.Add(context =>
        {
             // we can only do version completion if we have a package id
             if (context.ParseResult.GetValue(def.AddCommand.PackageIdArgument) is { HasVersion: false } packageId)
             {
                 // we should take --prerelease flags into account for version completion
                 var allowPrerelease = context.ParseResult.GetValue(def.AddCommand.PrereleaseOption);
        ...
        def.AddCommand.SetAction(parseResult => new PackageAddCommand(parseResult).Execute());
