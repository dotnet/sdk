...
internal static class CommonRunHelpers
{
    ...
    public static string? TryFindLaunchSettings(string projectOrEntryPointFilePath, string? launchProfile, Action<string, bool> report)
    {
        ...
        if (hasLaunchSetttings)
        {
            if (hasRunJson)
            {
                report(string.Format(CliCommandStrings.RunCommandWarningRunJsonNotUsed, runJsonPath, launchSettingsPath), false);
            }
        ...
        {
            report(string.Format(CliCommandStrings.RunCommandExceptionCouldNotLocateALaunchSettingsFile, launchProfile, $"""
                    {launchSettingsPath}
                    {runJsonPath}
                    """), true);
        }
