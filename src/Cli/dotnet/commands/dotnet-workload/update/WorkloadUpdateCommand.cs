...
namespace Microsoft.DotNet.Workloads.Workload.Update
{
    internal class WorkloadUpdateCommand : InstallingWorkloadCommand
    {
        ...
        private void WriteSDKInstallRecordsForVSWorkloads()
        {
            ...
            {
                // Do this gross check to avoid adding an unused & unnecessary method to FileBasedInstallers
                if(typeof(NetSdkMsiInstallerClient) == _workloadInstaller.GetType())
                {
        ...
        private async Task<List<string>> GetUpdatablePackageUrlsAsync(bool includePreview, IReporter reporter = null, INuGetPackageDownloader packageDownloader = null)
        {
            reporter ??= Reporter;
