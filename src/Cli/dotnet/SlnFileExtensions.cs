...
{
    public static void AddSolutionItem(this SlnFile slnFile, string fullFilePath, IList<string> solutionFolders)
    {
        ...
        {
            foreach (var solutionFolderName in solutionFolders)
            {
                var solutionFolder = slnFile.GetOrCreateSolutionFolder(solutionFolderName);
                slnFile.AddSolutionItemToSolutionFolder(solutionFolder, relativeFilePath);
            }
        }
        else
        {
            var defaultSolutionFolder = slnFile.GetOrCreateSolutionFolder();
            slnFile.AddSolutionItemToSolutionFolder(defaultSolutionFolder, relativeFilePath);
        }
    }

    private static void AddSolutionItemToSolutionFolder(
        this SlnFile slnFile,
        SlnProject solutionFolder,
        string relativeFilePath)
    {
        ...
        if (!solutionItems.TryGetValue(relativeFilePath, out var value))
        {
            solutionItems.Add(relativeFilePath, relativeFilePath);
            solutionItemsSection.SetContent(solutionItems);

            Reporter.Output.WriteLine(string.Format(CommonLocalizableStrings.SolutionItemAddedToTheSolution, relativeFilePath));
        }
        else
        {
            // TODO: use the value above?
