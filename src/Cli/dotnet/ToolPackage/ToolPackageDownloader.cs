...
namespace Microsoft.DotNet.Cli.ToolPackage
{
    internal class ToolPackageDownloader
    {
        ...
        protected DirectoryPath _tempStageDir;
        private readonly IToolPackageStore _toolPackageStore;
        protected readonly String _tempDirForAssetJson;
        protected readonly string _dirForLocalTool;

        public ToolPackageDownloader(
            IToolPackageStore store = null,
            //INuGetPackageDownloader nugetPackageDownloader = null,
            string tempDirPath = null
        )
        {
            _dirForLocalTool = "C:\\Users\\liannie\\.nuget\\packages\\";
            _toolPackageStore = store;
            _tempPackagesDir = new DirectoryPath(tempDirPath ?? PathUtilities.CreateTempSubdirectory());
            _tempDirForAssetJson = PathUtilities.CreateTempSubdirectory();    
        }
            ...
            )
        {
            ...
            _nugetPackageDownloader = new NuGetPackageDownloader.NuGetPackageDownloader(_tempStageDir);
        ...
        public async Task<IToolPackage> InstallPackageAsyncToExternalManagedLocation(PackageLocation packageLocation,
        PackageId packageId,
        VersionRange versionRange = null,
        string targetFramework = null,
        string verbosity = null
        )
        {
            ...
            _nugetPackageDownloader = new NuGetPackageDownloader.NuGetPackageDownloader(new DirectoryPath(_dirForLocalTool));
            ...
            using (FileStream packageStream = File.OpenRead(packagePath))
            {
                ...
                var packageHash = Convert.ToBase64String(new CryptoHashProvider("SHA512").CalculateHash(reader.GetNuspec()));
                var hashPath = new VersionFolderPathResolver(_dirForLocalTool).GetHashPath(packageId.ToString(), version);
            ...
            // Extract the package
            var nupkgDir = Path.Combine(_dirForLocalTool, packageId.ToString(), version.ToString());
            ...
            // Create asset file in corresponding folder
            ...
            //  Create NuGetv3LocalRepository
            NuGetv3LocalRepository localRepository = new(_dirForLocalTool);
            var package = localRepository.FindPackage(packageId.ToString(), version);
            ...
            lockFileTarget.Libraries.Add(lockFileLib);
            lockFile.Targets.Add(lockFileTarget);
            new LockFileFormat().Write(Path.Combine(_tempDirForAssetJson, "project.assets.json"), lockFile);

            return new ToolPackageInstance(id: packageId,
                            version: version,
                            packageDirectory: new DirectoryPath(_dirForLocalTool),
                            assetsJsonParentDirectory: new DirectoryPath(_tempDirForAssetJson));
        }
    } 
}
