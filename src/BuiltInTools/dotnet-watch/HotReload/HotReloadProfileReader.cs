...
namespace Microsoft.DotNet.Watcher.Tools
{
    public static class HotReloadProfileReader
    {
        public static HotReloadProfile InferHotReloadProfile(ProjectGraph projectGraph, IReporter reporter)
        {
            ...
            while (queue.Count > 0)
            {
                ...
                foreach (var item in projectCapability)
                {
                    ...
                    else if (item.EvaluatedInclude == "WebAssembly")
                    {
                        ...
                        {
                            reporter.Verbose($"HotReloadProfile: BlazorHosted. {aspnetCoreProject.FullPath} references BlazorWebAssembly project {currentNode.ProjectInstance.FullPath}.", emoji: "\u1F525");
                            return HotReloadProfile.BlazorHosted;
                        }

                        reporter.Verbose("HotReloadProfile: BlazorWebAssembly.", emoji: "\u1F525");
            ...
            reporter.Verbose("HotReloadProfile: Default.", emoji: "\u1F525");
