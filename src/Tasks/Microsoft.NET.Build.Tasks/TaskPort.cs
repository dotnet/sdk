...
namespace Microsoft.Build.Tasks.Windows
{
    ...
    ///
    /// </summary>
    public sealed class TaskPort : Task
#if NET10_0_OR_GREATER
...
#endif
    {
        ...
        {
#if NET10_0_OR_GREATER
            // Convert relative paths to absolute paths for multi-threading support
            _currentProject = TaskEnvironment.GetAbsolutePath(_currentProject);
            _intermediateOutputPath = TaskEnvironment.GetAbsolutePath(_intermediateOutputPath);
            if (!string.IsNullOrEmpty(TemporaryTargetAssemblyProjectName))
            {
                TemporaryTargetAssemblyProjectName = TaskEnvironment.GetAbsolutePath(TemporaryTargetAssemblyProjectName);
            }
#endif
    ...
    /// <summary>
    /// Represents an SDK reference with name, version, and minimum version.
    /// </summary>
    internal readonly struct SdkReference
    {
        ...
        /// <summary>
        /// Parses an SDK reference string in the format "Name/Version" or "Name/min=MinVersion".
        /// </summary>
        public static bool TryParse(string sdk, out SdkReference sdkReference)
        {
            ...
            int slashIndex = sdk.IndexOf('/');
            if (slashIndex >= 0)
            {
                name = sdk.Substring(0, slashIndex);
                string versionPart = sdk.Substring(slashIndex + 1);

                if (versionPart.StartsWith("min=", StringComparison.OrdinalIgnoreCase))
                {
                    minimumVersion = versionPart.Substring(4);
                }
                else
                {
                    version = versionPart;
