...
public class GenerateStaticWebAssetsDevelopmentManifest : Task, IMultiThreadableTask
{
    ...
    private IEnumerable<SegmentsAssetPair> ComputeManifestAssets(IEnumerable<StaticWebAsset> assets)
    {
        ...
        {
            // See the note on ChooseNearestAssetKind and in GenerateStaticWebAssetEndpointsManifest: this method yields more
            // than one asset only for a malformed manifest (a duplicate target path). Surface a targeted BLAZOR107 diagnostic
            // naming the colliding assets and their provenance instead of throwing an opaque exception from SingleOrDefault.
            var matchingAssets = StaticWebAsset.ChooseNearestAssetKind(group, StaticWebAsset.AssetKinds.Build).ToArray();
