...
public class GenerateStaticWebAssetEndpointsManifest : Task, IMultiThreadableTask
{
    ...
    private void UpdateExclusionPatternsCache(string existingPatternString, string patternString, string manifestAbsolutePath, string exclusionCacheAbsolutePath)
    {
    ...
    private IEnumerable<TargetPathAssetPair> ComputeManifestAssets(IEnumerable<StaticWebAsset> assets, string kind)
    {
        ...
        {
            // ChooseNearestAssetKind yields more than one asset only when the manifest is malformed (for example two `All`
            // assets, or an `All` asset colliding with a same-kind asset for the same target path). That situation is almost
            // always produced upstream by a referenced project or NuGet package, so surface a targeted diagnostic that names
            // the colliding assets and their provenance instead of letting the downstream `SingleOrDefault` throw an opaque
            // "Sequence contains more than one element" exception that points at this task rather than the real producer.
            var matchingAssets = StaticWebAsset.ChooseNearestAssetKind(group, kind).ToArray();

            if (matchingAssets.Length > 1)
            {
                Log.LogError(null, "BLAZOR107", "", null, 0, 0, 0, 0,
                    "More than one static web asset matched the target path '{0}' for asset kind '{1}'. " +
                    "Each target path must resolve to a single asset for a given asset kind. This usually means a " +
                    "referenced project or NuGet package produced an asset that collides with one generated by this " +
                    "project. Fix the project or package that produces one of these assets so that only a single asset " +
                    "maps to this target path:{2}",
                    group.Key,
                    kind,
                    string.Concat(matchingAssets.Select(a => Environment.NewLine +
                        $"  - Identity='{a.Identity}', SourceId='{a.SourceId}', SourceType='{a.SourceType}', AssetKind='{a.AssetKind}', AssetMode='{a.AssetMode}'")));
                continue;
            }

            var asset = matchingAssets.Length == 1 ? matchingAssets[0] : null;
