...
**2.6 — `P` smoke-tests the replacement.** `P` runs `D/dotnetup.exe --version`, waits synchronously for up to `X_V`, and requires status `0` and nonempty parseable version output matching the selected release's full version. This tests startup and release consistency, not authenticated freshness.
