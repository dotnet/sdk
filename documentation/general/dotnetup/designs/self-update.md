...
the [version metadata contract](version-metadata.md) for the current behavior and limitations.
...
The approach below applies to both stages unless noted. Stage A uses immediate failure at the `non-safe` gate; Stage B adds bounded waiting and forwarding using the same locks and loaded version/RID check. Signed version manifests and monotonic update authorization are deferred to future stages: the current unsigned metadata, checksums, and startup smoke check do not authenticate freshness or prevent downgrade/replay.
...
Let `V_channel` be the full version and RID resolved from the selected channel, and let `V_installed` be the readable version/RID metadata in `D/dotnetup.exe`. The [version metadata contract](version-metadata.md) preserves both fields exactly, without an independent digest or sidecar. Equality is local consistency, not content identity or authenticated freshness; same-version/RID rebuilds cannot be distinguished.
...
| `N` | `U` | exclusive | optional cleanup only, after passing the gate and version metadata check; one nonblocking attempt |
...
Because `S` holds neither lock, a self update can complete underneath it. `S` must cache image-derived values at startup, particularly the executable path and loaded version metadata. Capturing a path does not identify the loaded image: the metadata comes from the loaded record, never by reopening that path. See [Program](../../../../src/Installer/dotnetup.Library/Program.cs) and the [loaded-record contract](version-metadata.md#build-and-runtime-access).
...
**Rule 1 — `P` acquires `U` before `A`.** `U` is taken first so that `P` does not exclude every `N` while waiting out a peer `self update` or cleanup. `N` acquires only `A` at the gate; after passing its version metadata check, it may also hold `U` briefly for cleanup under step 2.9.
...
Reading the canonical version metadata without holding `U` is acceptable here and is not in step 2.9, because the two reads gate different actions. Step 2.9 uses the value to delete backups, which is irreversible, so it reads under `U`. Step 1.0 uses the value only to decide whether to continue; a transiently absent or unreadable canonical path cannot authorize replacement or cleanup.
...
**1.4 — `N` checks version metadata.** After acquiring `A` shared, `N` compares its cached `DotnetupVersionMetadata.Current`, read from its loaded image, with `V_installed`, read directly from the canonical executable's embedded record while retaining `A`. The [version metadata contract](version-metadata.md) defines the record, automatic build generation, and offline reader. This check launches no child and never uses a pathname lookup to identify the loaded image.
...
`P` also reads the staged executable's embedded full version/RID and requires it to equal `V_channel` before replacement. This is an unsigned consistency check, not authorization.
...
**2.6 — `P` smoke-tests the replacement.** `P` runs `D/dotnetup.exe --version`, waits synchronously for up to `X_V`, and requires status `0` and nonempty parseable version output. This tests startup, not authenticated freshness.
...
The verifier disables telemetry and suppresses the first-run banner in the child environment, captures bounded stdout/stderr, and retains cancellation and termination timeouts. `--version` follows ordinary startup; there is no separate hidden identity action. Exact version/RID consistency was checked offline before replacement. Executing the replacement remains necessary to detect startup failure.
...
Dotnetup stages on the destination volume and uses `File.Replace`, avoiding a cross-volume copy or deliberately split forward moves without promising a gapless canonical path. Dotnetup checks full version/RID consistency offline before replacement, then uses `--version` as a startup smoke check. Neither check authorizes release freshness. Aspire takes no cross-process lock, so concurrent self-updates race over the canonical path and the backups, and nothing stops another Aspire command from running across the replacement — the two problems `U` and `A` exist to solve.
...
The rollback move restores the old executable and its version metadata. Step 1.4 permits an `N` loaded from that version/RID to proceed and rejects or forwards one loaded from the rejected version, exactly as on Windows.
