...
The main use for primary outputs is to provide the list of files to perform post actions implemented by the host. dotnet CLI host and Visual Studio host implement the default set of post actions based on primary outputs returned by API, described in [post-action registry](https://github.com/dotnet/sdk/blob/main/documentation/TemplateEngine/Post-Action-Registry.md). In case the template files have to be used in post actions, they should be added as the primary outputs in `template.json` definition.
