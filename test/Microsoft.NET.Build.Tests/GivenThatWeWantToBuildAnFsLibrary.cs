...
{
    public class GivenThatWeWantToBuildAnFsLibrary : SdkTest
    {
       [Fact]
        public void It_builds_the_library_successfully()
        {
            var testAsset = _testAssetsManager
                .CopyTestAsset("FsAppWithLibrary")
                .WithSource()
       ...
       [Fact]
            ...
            Action<XDocument> projectChanges = null)
        {
            ...
            var testAsset = testAssetsManager
                .CopyTestAsset("FsAppWithLibrary", callingMethod)
                .WithSource();
        ...
        private TestAsset CreateDocumentationFileLibraryAsset(bool? generateDocumentationFile, string documentationFile, [CallerMemberName] string callingMethod = "")
        {
            ...
            var testAsset = _testAssetsManager
                .CopyTestAsset("FsAppWithLibrary", callingMethod, identifier)
                .WithSource()
        ...
        public void The_design_time_build_succeeds_before_nuget_restore()
        {
            ...
            string fsharpDesignTimeTargets = Path.Combine(vsInstallDir, @"MSBuild\Microsoft\VisualStudio\Managed\Microsoft.FSharp.DesignTime.targets");
            ...
            var libraryProjectDirectory = Path.Combine(testAsset.TestRoot, "TestLibrary");
            var projectFile = Path.Combine(libraryProjectDirectory, "TestLibrary.fsproj");
            ...
            var args = new[]
            {
                ...
                $"/p:FSharpDesignTimeTargetsPath={fsharpDesignTimeTargets}",
        ...
        {
            var testAsset = _testAssetsManager
                .CopyTestAsset("FsAppWithLibrary", "ImplicitConfigurationConstants", configuration)
                .WithSource()
                .Restore(relativePath: "TestLibrary");
        ...
        public void It_implicitly_defines_compilation_constants_for_the_target_framework(string targetFramework, string[] expectedDefines, bool buildOnlyOnWindows)
        {
            ...
            var testAsset = _testAssetsManager
                .CopyTestAsset("FsAppWithLibrary", "ImplicitFrameworkConstants", targetFramework)
                .WithSource()
                .WithProjectChanges(project =>
                {
        ...
        {
            var testAsset = _testAssetsManager
                .CopyTestAsset("FsAppWithLibrary", "EmptyTargetFramework")
                .WithSource()
                .WithProjectChanges(project => 
                {
                    project.Root
        ...
        {
            var testAsset = _testAssetsManager
                .CopyTestAsset("FsAppWithLibrary", "RidlessLib")
                .WithSource()
                .Restore(relativePath: "TestLibrary");
