...
namespace Microsoft.DotNet.Cli.Test.Tests
{
    public class GivenDotnetTestBuildsAndRunsTestFromCsproj : SdkTest
    {
        ...
        [Fact]
        public void ItTestsWithSelfContainedOption()
        {
            var testInstance = _testAssetsManager.CopyTestAsset("XunitCore")
                            .WithSource()
                            .WithVersionVariables();
            ...
            var result = new DotnetTestCommand(Log, disableNewOutput: true, ConsoleLoggerOutputNormal)
                .WithWorkingDirectory(rootPath)
                .Execute("--runtime", rid, "--self-contained");

            result
                .Should()
                .NotHaveStdErrContaining("NETSDK1179")
                .And
                .NotHaveStdErrContaining("MSB1001");
        ...
        [Fact]
        public void ItTestsWithNoSelfContainedOption()
        {
            var testInstance = _testAssetsManager.CopyTestAsset("XunitCore")
                            .WithSource()
                            .WithVersionVariables();
            ...
            var result = new DotnetTestCommand(Log, disableNewOutput: true, ConsoleLoggerOutputNormal)
                .WithWorkingDirectory(rootPath)
                .Execute("--runtime", rid, "--no-self-contained");
        ...
        [Fact]
        public void ItFailsWhenBothSelfContainedAndNoSelfContainedAreSpecified()
        {
            var testInstance = _testAssetsManager.CopyTestAsset("XunitCore")
                            .WithSource()
                            .WithVersionVariables();
            ...
            var result = new DotnetTestCommand(Log, disableNewOutput: true, ConsoleLoggerOutputNormal)
                .WithWorkingDirectory(rootPath)
                .Execute("--runtime", rid, "--self-contained", "--no-self-contained");
            ...
            if (!TestContext.IsLocalized())
            {
                result.StdErr.Should().Contain("--self-contained")
                    .And.Contain("--no-self-contained");
            }
        }
