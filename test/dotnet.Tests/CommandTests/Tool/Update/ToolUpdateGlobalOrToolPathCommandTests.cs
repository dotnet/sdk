...
namespace Microsoft.DotNet.Tests.Commands.Tool
{
    ...
    public class ToolUpdateGlobalOrToolPathCommandTests : SdkTest
    {
        ...
        {
            CreateInstallCommand($"-g {_packageId} --version {LowerPackageVersion}").Execute(CancellationToken.None);
        ...
        {
            CreateInstallCommand($"-g {_packageId} --version {LowerPackageVersion}").Execute(CancellationToken.None);
            CreateInstallCommand($"-g {_packageId2} --version {LowerPackageVersion}", _packageId2.ToString()).Execute(CancellationToken.None);

            CreateUpdateCommand($"--all -g -v:d").Execute(CancellationToken.None);
        ...
        public void GivenAnExistedLowerversionInstallationWhenCallFromRedirectorItCanUpdateThePackageVersion()
        {
            ...
            toolUpdateCommand.Execute(CancellationToken.None);
        ...
        {
            CreateInstallCommand($"-g {_packageId} --version {HigherPackageVersion}").Execute(CancellationToken.None);
            _reporter.Lines.Clear();
        ...
        {
            CreateInstallCommand($"-g {_packageId} --version {HigherPreviewPackageVersion}").Execute(CancellationToken.None);
            _reporter.Lines.Clear();
