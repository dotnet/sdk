...
{
    /// <summary>
    /// Test infrastructure that simulates how MSBuild multi-threaded execution
    ...
    ///
    /// The injected <see cref="TaskEnvironment"/> is built with MSBuild's real
    ...
    /// API and represents the multi-threaded mode (isolated working directory + env snapshot).
    /// </summary>
    internal sealed class TaskTestEnvironment : IDisposable
    {
        ...
        /// <summary>
        /// A multi-threaded-mode <see cref="TaskEnvironment"/> whose <c>ProjectDirectory</c> points to
        /// <see cref="ProjectDirectory"/>. Path resolution against this environment is independent of
        /// the process CWD, mirroring what MSBuild injects into <see cref="IMultiThreadableTask"/>
        /// instances in MT execution mode.
        /// </summary>
