...
public class GenerateStaticWebAssetEndpointsManifestTest
{
    ...
    [Fact]
    public void GenerateManifest_WhenMultipleAllAssetsCollideOnTargetPath_FailsWithBlazor107Diagnostic()
    {
        // Regression test for https://github.com/dotnet/sdk/issues/54779.
        // A MAUI Blazor Hybrid app that references a Razor class library AND the BlazorWebView package ends up
        // with three assets mapping to the same target path "_framework/blazor.modules.json":
        //   * an `All` fallback shipped by the Microsoft.AspNetCore.Components.WebView package,
        //   * an `All` build JS modules manifest generated by the app, and
        //   * a `Publish` publish JS modules manifest generated by the app.
        // ChooseNearestAssetKind surfaces the two ambiguous `All` assets (stopping before the more specific
        // `Publish` asset), which used to make the downstream SingleOrDefault throw an opaque "Sequence contains
        // more than one element" exception that pointed at this task instead of the package that actually
        // produced the colliding asset. The task must instead fail with a targeted diagnostic that names the
        // colliding assets and their provenance so the fix can be made where the duplicate asset is produced.
        var errors = new List<BuildErrorEventArgs>();
        var buildEngine = new Mock<IBuildEngine>();
        buildEngine.Setup(e => e.LogErrorEvent(It.IsAny<BuildErrorEventArgs>()))
            .Callback<BuildErrorEventArgs>(errors.Add);

        var packageFallback = CreateAsset(
            "blazor.modules.json",
            sourceId: "Microsoft.AspNetCore.Components.WebView",
            sourceType: StaticWebAsset.SourceTypes.Package,
            relativePath: "_framework/blazor.modules.json",
            assetKind: StaticWebAsset.AssetKinds.All,
            basePath: "/");

        var appBuildManifest = CreateAsset(
        ...
        var appPublishManifest = CreateAsset(
        ...
        // The two `All` assets must be encountered before the `Publish` asset; that ordering is what triggers
        // the ambiguity in ChooseNearestAssetKind and matches the failing build reported in #54779.
        var assets = new[] { packageFallback, appBuildManifest, appPublishManifest };
        var path = Path.Combine(AppContext.BaseDirectory, Guid.NewGuid().ToString("N") + "endpoints.json");
        ...
        var task = new GenerateStaticWebAssetEndpointsManifest
        {
            Assets = assets.Select(a => a.ToTaskItem()).ToArray(),
            Endpoints = [],
            ManifestType = StaticWebAsset.AssetKinds.Publish,
            Source = "MyApp",
            ManifestPath = path,
        ...
        try
        {
            ...
            error.Code.Should().Be("BLAZOR107");
            error.Message.Should().Contain("_framework/blazor.modules.json");
            // The diagnostic must surface the provenance of every colliding asset so the user is directed to the
            // producing project/package (here the WebView package) instead of to this task.
            error.Message.Should().Contain(packageFallback.Identity);
            error.Message.Should().Contain(appBuildManifest.Identity);
            error.Message.Should().Contain("Microsoft.AspNetCore.Components.WebView");
        }
        finally
        {
            if (File.Exists(path))
    ...
    [Fact]
    public void GenerateManifest_WhenAllAndPublishAssetShareTargetPath_PicksPublishWithoutDiagnostic()
    {
        // Guards the BLAZOR107 diagnostic against false positives: a single `All` asset colliding with a more
        // specific `Publish` asset for the same target path is NOT ambiguous -- ChooseNearestAssetKind picks the
        // `Publish` asset. This is the common, legitimate case and must keep working without an error, exactly as
        // the previous SingleOrDefault() did (it only threw when more than one asset survived selection).
        var errors = new List<BuildErrorEventArgs>();
        var buildEngine = new Mock<IBuildEngine>();
        buildEngine.Setup(e => e.LogErrorEvent(It.IsAny<BuildErrorEventArgs>()))
            .Callback<BuildErrorEventArgs>(errors.Add);
        ...
        var assets = new[] { appBuildManifest, appPublishManifest };
        var path = Path.Combine(AppContext.BaseDirectory, Guid.NewGuid().ToString("N") + "endpoints.json");
