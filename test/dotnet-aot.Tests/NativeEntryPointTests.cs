...
public class NativeEntryPointTests : IDisposable
{
    ...
    private readonly object? _originalHostfxrPath;

    public NativeEntryPointTests()
    {
        _originalEnableAot = Environment.GetEnvironmentVariable("DOTNET_CLI_ENABLEAOT");
        _originalHostfxrPath = AppContext.GetData("HOSTFXR_PATH");
    }

    public void Dispose()
    {
        // Restore original env var
        Environment.SetEnvironmentVariable("DOTNET_CLI_ENABLEAOT", _originalEnableAot);

        // Restore original AppContext data
        AppContext.SetData("HOSTFXR_PATH", _originalHostfxrPath);
    }
