...
public class CompilationHandlerTests(ITestOutputHelper logger) : DotNetWatchTestBase(logger)
{
    ...
    public async Task ReferenceOutputAssembly_False()
    {
        ...
        var projectGraph = MSBuildFileSetFactory.TryLoadProjectGraph(options.ProjectPath, globalOptions: [], reporter, projectGraphRequired: false, CancellationToken.None);
