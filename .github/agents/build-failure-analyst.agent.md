...
- **Key projects**: `dotnet` CLI host and its sub-commands (under `src/Cli`), `Microsoft.DotNet.Cli.Utils`, the `dotnet test`/`dotnet pack`/`dotnet build` command implementations, and the `redist` project that produces the SDK layout. Tests live under `test/` (notably `test/dotnet.Tests/`).
- **Local toolchain**: builds use the repo-local SDK at `./.dotnet/`. Errors mentioning a missing SDK version usually mean `global.json` drifted from what's in `.dotnet/`.
- **Generated templates**: `documentation/manpages/sdk/**` and parts of `template_feed/**` are generated — do not propose hand-edits to those files.
- **External boundary**: changes to template engine packages (`Microsoft.TemplateEngine.*`) belong in `dotnet/templating`, not here. Flag any suggestion that tries to work around a template-engine issue inside this repo.
- **Localization**: never propose hand-edits to `.xlf` files. If a `.resx` change broke an `.xlf`, the fix is regenerating via the `/t:UpdateXlf` MSBuild target on the owning project.
...
| ------- | ------------------------- | ------------------ |
| Missing API / using directive | `CS0103`, `CS0246`, `CS0234` | Removed namespace, missing project reference, missing NuGet package, missing TFM-conditional code. SDK frequently splits implementation across `#if NET` / `#if NETFRAMEWORK` blocks — check for missing guards. |
...
| NuGet resolution failure | `NU####`, `NETSDK####` | Package not found, version conflict, TFM not supported. **Use the NuGet MCP server** to resolve. Versions usually live in `eng/Versions.props`. |
| Localization regression | `xlf` parsing error, `LCMessages` | `.resx` modified without rebuild; never hand-edit `.xlf`. Fix: `/t:UpdateXlf` on the owning project. |
| Manpage / template-feed | edits under `documentation/manpages/sdk/**` | Generated files — do not propose hand-edits, suggest regenerating instead. |
...
| ---- | ------- | --------- | ------- |
| `CS0103` | `dotnet` | `Foo.cs:42` | The name 'foo' does not exist… |
...
- Do not propose hand-edits to generated files (`documentation/manpages/sdk/**`, `.xlf` files); suggest regeneration instead.
- For changes that look like they require modifying `Microsoft.TemplateEngine.*` package internals, flag that the fix belongs in `dotnet/templating` rather than this repo.
