...
| [`src/Installer`](../../src/Installer) | dotnetup NativeAOT entry point, command library, and installation library. See the [self-update design](../../documentation/general/dotnetup/designs/self-update.md) and [version-query contract](../../documentation/general/dotnetup/designs/self-update-verification.md). | Self-update uses bounded `--version` children for installed-version queries and startup verification; rollback uses transaction state, not an embedded record. Tested by `test/dotnetup.Tests`. |
