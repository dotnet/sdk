...
| [`src/Installer`](../../src/Installer) | dotnetup NativeAOT entry point, command library, and installation library. See the [self-update review guide](../../documentation/general/dotnetup/designs/self-update.md#review-guide) and [build-identity tooling](../../src/Installer/BuildIdentity/README.md). | The host-only identity task source-links the Installation reader without a product project reference; consumed by native generation/publish validation and `test/dotnetup.Tests`. |
