...
"$HELIX_WORKITEM_ROOT/.dotnet/dotnet" run --no-build --project $HELIX_WORKITEM_ROOT\src\Tests\PerformanceTestsResultGenerator\PerformanceTestsResultGenerator.csproj -- --file-option "$perfWorkingDirectory/Microsoft.BenchView.JSONFormat/tools/measurement.py"
