...
function InstallBootstrapSdkWithDotnetup() {
    ...
    if ((-not $restore) -or
        ((Test-Path variable:fromVMR) -and $fromVMR) -or
        $env:DotNetBuildFromSource -eq "true" -or
        ((Test-Path variable:productBuild) -and $productBuild) -or
        ((Test-Path variable:properties) -and $properties -like "*DotNetBuildRepo=true*") -or
        [string]::IsNullOrEmpty($dotnetSdkVersion)) {
        return
    }
