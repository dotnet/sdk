...
try {
    ...
    if ($real) {
        Invoke-Expression 'dotnet run -c Release -f net7.0 --runtimes net7.0 --project dotnet-format.Performance.csproj -- --memory --join --filter Microsoft.CodeAnalysis.Tools.Perf.Real*'
        exit 0
    }
    
    if ($all) {
        Invoke-Expression 'dotnet run -c Release -f net7.0 --runtimes net7.0 --project dotnet-format.Performance.csproj -- --memory --join --filter *'
        exit 0
    }
}
catch {
