...
function InstallBootstrapSdkWithDotnetup {
  ...
  if [[ "$restore" != true || "${from_vmr:-false}" == true || "${properties:-}" == *"ArcadeBuildFromSource=true"* || "${product_build:-false}" == true || "${properties:-}" == *"DotNetBuildRepo=true"* || -z "$dotnet_sdk_version" ]]; then
    return
  fi
  ...
  if IsMusl; then
    echo "Using the dotnet-install fallback for SDK acquisition on musl."
    return
  fi
